...
def run_worker():
    ...
    while True:
        try:
            ...
            if not raw_articles:
                idle_count += 1
                sleep_time = 10 if idle_count < 10 else 30
                logger.info(f"No new articles found. Sleeping for {sleep_time}s...")
            ...
            for raw in raw_articles:
                try:
                    ...
                    existing_pg = pg_session.query(Article).filter(Article.original_url == raw.url).first()
                    if existing_pg:
